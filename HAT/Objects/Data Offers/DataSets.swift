...
public struct DataSets: HATObject {
    ...
    public init(dictionary: Dictionary<String, JSON>) {
        ...
        if let tempName: String = dictionary[DataSets.CodingKeys.name.rawValue]?.string {
            
            name = tempName
        }
        
        if let tempDataSetsDescription: String = dictionary[DataSets.CodingKeys.dataSetsDescription.rawValue]?.string {
            
            dataSetsDescription = tempDataSetsDescription
        }
        
        if let tempFields: [JSON] = dictionary[DataSets.CodingKeys.fields.rawValue]?.array, !tempFields.isEmpty {
