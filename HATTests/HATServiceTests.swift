...
internal class HATServiceTests: XCTestCase {
    ...
    func testGetAppToken() {
        ...
        HATService.getApplicationTokenLegacyFor(serviceName: serviceName, userDomain: userDomain, userToken: token, resource: resource, succesfulCallBack: completion, failCallBack: failed)
    ...
    func testPurchase() {
        ...
        purchaseModel.hatName = "mariostsekis"
        purchaseModel.password = "veryGoodPassword"
        ...
        func completion(message: String, userToken: String?) {
            
            XCTAssertEqual(message, "purchase ok")
            expectationTest.fulfill()
        }
